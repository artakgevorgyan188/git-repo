...
class CartController extends AppController
{
    ...
    public function actionView()
    {
        ...
        if ($order->load(Yii::$app->request->post()) && $order->validate()) {
            ...
            $order->address = Html::encode($order->address);
            if ($order->save()) {
                ...
                Yii::$app->mailer->compose('order', ['session' => $session])
                    ...
                    ->setFrom(['artakgevorgyan188@mail.ru'=>'yiishopper.local'])//"yiishopper.local" <artakgevorgyan188@mail.ru> with our domain or we can try this setFrom('artakgevorgyan188@mail.ru')
                    ->setTo($order->email)//if we send admin setTo(Yii::$app->params['adminEmail']) in config -> params
                    ->setSubject('Заказ сайта...')
                    ->send();
                Yii::$app->mailer->compose('order', ['session' => $session])
                    ->setFrom(['artakgevorgyan188@mail.ru'=>'yiishopper.local'])
